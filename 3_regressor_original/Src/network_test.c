...
#define AI_NET_OBJ_INSTANCE g_network_test
 
#undef AI_NETWORK_TEST_MODEL_SIGNATURE
#define AI_NETWORK_TEST_MODEL_SIGNATURE     "ddd8804313e8814d83c4c96fb2bcfb4a"
...
#define AI_TOOLS_DATE_TIME   "Mon Feb 10 18:25:50 2020"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_NETWORK_TEST_N_BATCHES
#define AI_NETWORK_TEST_N_BATCHES         (1)
...
AI_NETWORK_OBJ_DECLARE(
  ...
  AI_BUFFER_OBJ_INIT(AI_BUFFER_FORMAT_U8,
                     1, 1, 0, 1,
                     NULL),
  AI_TENSOR_LIST_IO_ENTRY(AI_FLAG_NONE, AI_NETWORK_TEST_IN_NUM, &input_0_output),
  AI_TENSOR_LIST_IO_ENTRY(AI_FLAG_NONE, AI_NETWORK_TEST_OUT_NUM, &dense_1_output),
...
AI_DECLARE_STATIC
ai_bool network_test_configure_weights(
...
AI_API_ENTRY
ai_bool ai_network_test_get_info(
  ai_handle network, ai_network_report* report)
{
  ...
  {
    ai_network_report r = {
      .model_name        = AI_NETWORK_TEST_MODEL_NAME,
      .model_signature   = AI_NETWORK_TEST_MODEL_SIGNATURE,
...
AI_API_ENTRY
ai_error ai_network_test_get_error(ai_handle network)
{
  return ai_platform_network_get_error(network);
}

AI_API_ENTRY
ai_error ai_network_test_create(
...
AI_API_ENTRY
ai_handle ai_network_test_destroy(ai_handle network)
{
  return ai_platform_network_destroy(network);
}

AI_API_ENTRY
ai_bool ai_network_test_init(
  ai_handle network, const ai_network_params* params)
{
  ...
  ai_bool ok = true;
  ok &= network_test_configure_weights(net_ctx, &params->params);
...
AI_API_ENTRY
ai_i32 ai_network_test_forward(ai_handle network, const ai_buffer* input)
{
